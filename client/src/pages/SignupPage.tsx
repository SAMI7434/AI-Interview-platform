...
export function SignupPage({
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      const userEmail = (result as any)._tokenResponse.email;
      const uid = (result as any)._tokenResponse.user_id;
      const formData = {
        name: displayName,
        email: userEmail,
        firebaseUID: uid,
      ...
      const errorMessage = (error as any).response?.data?.error || (error as any).message || "An error occurred during signup";
      const newNotification: Notification = {
        id: Date.now().toString(),
        type: "error",
